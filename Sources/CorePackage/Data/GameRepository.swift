...
extension GameRepository: GameRepositoryProtocol {
    public func getListOfGames(page: Int, search: String) -> AnyPublisher<
        GameListModel, CommonError
    > {
        return Publishers.Zip(
            self.remoteDS.getListOfGames(page: page, search: search),
            self.localDS.fetchFavoriteGameEntityIds()
        )
    ...
    public func fetchFavoriteGamesFromLocal() -> AnyPublisher<
        [GameListItemModel], CommonError
    > {
        return self.localDS
            .fetchFavoriteGameEntity()
            .map { entities -> [GameListItemModel] in
                return entities.map { entity in
                    entity.toGameListItemModel()
                }
            }
    ...
    public func saveFavoriteGame(game: GameListItemModel) -> AnyPublisher<Void, Never> {
        return self.localDS
            .addFavoriteGameEntity(entity: game.toFavoriteGameEntity(model: game))
            .eraseToAnyPublisher()
    }
    
    public func deleteFavoriteGame(game: GameListItemModel) -> AnyPublisher<Void, Never> {
        return self.localDS
            .removeFavoriteGameEntity(entity: game.toFavoriteGameEntity(model: game))
            .receive(on: DispatchQueue.main)
            .eraseToAnyPublisher()
    }
}

public final class MockGameRepository: GameRepositoryProtocol {
    var isSimulateError = false
    
    public init(isSimulateError: Bool = false) {
        self.isSimulateError = isSimulateError
    }
    
    public func fetchFavoriteGamesFromLocal() -> AnyPublisher<[GameListItemModel], CommonError> {
    ...
    public func getListOfGames(page: Int, search: String) -> AnyPublisher<GameListModel, CommonError> {
        if isSimulateError {
